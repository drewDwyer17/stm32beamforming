...
//2 buffers for recieving data, one active and one standby
static uint8_t uart_rx_dma_buf[DMA_MAX_RX_CMD_LENGTH];
uint8_t uart_rx_process_buf[DMA_MAX_RX_CMD_LENGTH];

volatile bool uart_rx_data_pending = false; //flag for main loop
...
/*
 * IDLE interrupt (usart1_Isr) to fire
 int main(void)
{
    while (1) {
     if (uart_rx_data_pending) {
            ...
            processedHostCmdUart_s processedCmd;
...
void usart1_isr(void)
{
    if (usart_get_flag(UART_PORT, USART_ISR_IDLE)) {
        ...
        restart_dma_rx();
        uart_rx_data_pending = true; //set for main loop 
    }
}
